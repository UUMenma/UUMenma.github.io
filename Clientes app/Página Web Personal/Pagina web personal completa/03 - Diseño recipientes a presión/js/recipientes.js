function calcularepFcn() {
    ...
    Vtapas = Math.PI/12*(Math.pow(diametro + 2*espesortapareal,3) - Math.pow(diametro,3));
        ...
        pesoequipo = Math.trunc((Vtapas + Vcuerpo)*rhoMaterial);
        document.querySelector('#pesoequipo').value = pesoequipo;
        
        
        r = diametro/2;
        theta = Math.acos((r-nivel)/r);
        Vliquido = Math.pow(r,2)*longitud*(theta-Math.sin(theta)*Math.cos(theta));
        document.getElementById("volumenLiquido").value = Vliquido.toFixed(2);
        pesoLiquido = Math.trunc(Vliquido*rho);
        document.getElementById("pesoLiquido").value = pesoLiquido;
        pesoLleno = pesoLiquido+pesoequipo;
        document.getElementById("pesoLleno").value = pesoLleno;
